...
namespace ServerRaceMonitor
{
	public partial class Inicio : Form
	{
		...
		private void Inicio_Load(object sender, EventArgs e)
		{
			...
			if (backgroundWorkerUpdateWeb.IsBusy == false)
			{
				backgroundWorkerUpdateWeb.RunWorkerAsync();
			}
		}
		...
		private void TimerUpdateEstadisticas_Tick(object sender, EventArgs e)
		{
			...
			T = T + "\r\n" + "Actualizaciones Sitioweb Sowart: " + ActualizacionesSitioWeb.ToString();
		...
		private void backgroundWorkerUpdateWeb_DoWork(object sender, DoWorkEventArgs e)
		{
			System.Threading.Thread.CurrentThread.Name = "BackgroundWorkerUpdateWEB";
			while (true)
			{
				try
				{
					string filePath = "Video.xml";
					if (File.Exists(filePath))
					{
						DateTime lastWriteTime = File.GetLastWriteTime(filePath);

						// Si el archivo ha cambiado, se sube al servidor
						if (lastWriteTime > lastModified)
						{
							Console.WriteLine($"Cambio detectado en el archivo. Subiendo... {DateTime.Now}");

							if (UploadFile().Result)
							{
								lastModified = lastWriteTime;
								ActualizacionesSitioWeb++;
							}
						}
					}
					else
					{
						...
						{
							var response = client.GetAsync("http://www.sowart.com.ar/envivo/update.php").Result;

							if (response.IsSuccessStatusCode)
							{
								Console.WriteLine("Notificación GET enviada correctamente.");
								ActualizacionesSitioWeb++;
							}
							else
							{
				...
				{
					Console.WriteLine($"Error al monitorear el archivo: {ex.Message}");
				}
				Thread.Sleep(5000);
			}
		}
		...
		private static async Task<bool> UploadFile()
		{
			...
			try
			{
				string fileContent = File.ReadAllText(filePath); // Leer contenido  

				using (var client = new HttpClient())
				using (var content = new StringContent(fileContent, Encoding.UTF8, "text/plain"))
				{
					HttpResponseMessage response = await client.PostAsync("https://www.sowart.com.ar/envivo/update.php", content);
